...
func Test_strategyService_UpdateStrategyTickGroup(t *testing.T) {
	...
	tests := []struct {
		...
		arg1                     string
		want1                    error
		wantSetSymbolInfoHistory []interface{}
	}{
		{name: "戦略が見つからなければエラー",
		...
		{name: "呼値グループの更新に失敗したらエラー",
			strategyStore:            &testStrategyStore{GetByCode1: &Strategy{}, SetSymbolInfo1: ErrUnknown},
			kabusAPI:                 &testKabusAPI{GetSymbol1: &Symbol{TickGroup: TickGroupTopix100, TradingUnit: 100}},
			arg1:                     "strategy-code-001",
			want1:                    ErrUnknown,
			wantSetSymbolInfoHistory: []interface{}{"strategy-code-001", TickGroupTopix100, 100.0}},
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(test.want1, got1) || !reflect.DeepEqual(test.wantSetSymbolInfoHistory, test.strategyStore.SetSymbolInfoHistory) {
				t.Errorf("%s error\nwant: %+v, %+v\ngot: %+v, %+v\n", t.Name(), test.want1, test.wantSetSymbolInfoHistory, got1, test.strategyStore.SetSymbolInfoHistory)
			}
		})
	}
}
