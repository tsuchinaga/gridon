...
func Test_contractService_entryContract(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "戦略の最終約定情報保存に失敗したらエラー",
			positionStore: &testPositionStore{},
			strategyStore: &testStrategyStore{GetByCode1: &Strategy{}, SetBasePrice1: ErrUnknown},
			arg1:          &Order{Code: "order-code-001", StrategyCode: "strategy-code-001", SymbolCode: "1475", Exchange: ExchangeToushou, Side: SideBuy, Product: ProductMargin, MarginTradeType: MarginTradeTypeDay},
			arg2:          Contract{PositionCode: "position-code-001", Price: 2070, Quantity: 4, ContractDateTime: time.Date(2021, 10, 28, 10, 0, 0, 0, time.Local)},
			want1:         ErrUnknown,
			wantSaveHistory: []interface{}{&Position{
		...
		{name: "戦略に今回の約定データよりも新しいデータが登録されていたら、約定情報の更新をしない",
			positionStore: &testPositionStore{},
			strategyStore: &testStrategyStore{GetByCode1: &Strategy{BasePrice: 2080, BasePriceDateTime: time.Date(2021, 10, 28, 10, 1, 0, 0, time.Local)}},
			arg1:          &Order{Code: "order-code-001", StrategyCode: "strategy-code-001", SymbolCode: "1475", Exchange: ExchangeToushou, Side: SideBuy, Product: ProductMargin, MarginTradeType: MarginTradeTypeDay},
			arg2:          Contract{PositionCode: "position-code-001", Price: 2070, Quantity: 4, ContractDateTime: time.Date(2021, 10, 28, 10, 0, 0, 0, time.Local)},
			want1:         nil,
			wantSaveHistory: []interface{}{&Position{
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !errors.Is(got1, test.want1) ||
				...
				!reflect.DeepEqual(test.wantSetContractHistory, test.strategyStore.SetBasePriceHistory) {
				t.Errorf("%s error\nresult: %+v, %+v, %+v, %+v\nwant: %+v, %+v, %+v, %+v\ngot: %+v, %+v, %+v, %+v\n", t.Name(),
					...
					got1, test.positionStore.SaveHistory, test.strategyStore.AddStrategyCashHistory, test.strategyStore.SetBasePriceHistory)
			}
		})
	}
}
...
func Test_contractService_exitContract(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "戦略に、今回の約定データよりも新しいデータが登録されていたら、約定情報の更新をしない",
			positionStore: &testPositionStore{},
			strategyStore: &testStrategyStore{GetByCode1: &Strategy{BasePrice: 2080, BasePriceDateTime: time.Date(2021, 9, 29, 10, 1, 0, 0, time.Local)}},
			arg1: &Order{
				StrategyCode: "strategy-code-001",
				HoldPositions: []HoldPosition{
					{PositionCode: "position-code-001", HoldQuantity: 100, ContractQuantity: 80},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !errors.Is(got1, test.want1) ||
				...
				t.Errorf("%s error\nresult: %+v, %+v, %+v, %+v\nwant: %+v, %+v, %+v, %+v\ngot: %+v, %+v, %+v, %+v\n", t.Name(),
					...
					got1, test.positionStore.ExitContractHistory, test.strategyStore.AddStrategyCashHistory, test.strategyStore.SetBasePriceHistory)
			}
		})
	}
}
