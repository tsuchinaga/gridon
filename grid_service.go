...
func (s *gridService) Leveling(strategy *Strategy) error {
	...
	upper := s.tick.TickAddedPrice(TickGroupOther, basePrice, strategy.GridStrategy.NumberOfGrids*strategy.GridStrategy.Width)
	lower := s.tick.TickAddedPrice(TickGroupOther, basePrice, -1*strategy.GridStrategy.NumberOfGrids*strategy.GridStrategy.Width)
	...
	for i := 1; i <= strategy.GridStrategy.NumberOfGrids; i++ {
		// upper
		{
			upper := s.tick.TickAddedPrice(TickGroupOther, basePrice, i*strategy.GridStrategy.Width)
		...
		// lower
		{
			lower := s.tick.TickAddedPrice(TickGroupOther, basePrice, -1*i*strategy.GridStrategy.Width)
